...
# Monolithic Linking Stage: Fuses your local kernel with your graphics layers!
bin/antagonist.bin: $(KERNEL_OBJS) $(DISTRO_OBJS)
	mkdir -p bin
...
# Local Kernel compilation mapping rules
kernel/%.o: kernel/%.cpp
	$(CC) $(CFLAGS) -Iinclude/kernel $< -o $@

# Local User-space compilation mapping rules
